...
import { Zap, Settings, Guitar, Volume2, Trash2, GripVertical } from 'lucide-react'
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
function SortableGearItem({ 
  ...
  return (
    ...
    >
      ...
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${lessonMode ? 'bg-neon-cyan/20 text-neon-cyan' : 'bg-neon-purple/20 text-neon-purple'}`}>
        {index + 1}
      </div>
...
function DragOverlayContent({ 
  ...
  return (
    <div 
      className="flex items-center gap-4 p-3 rounded-lg border bg-slate-800/95 backdrop-blur-xl border-neon-blue/50 shadow-2xl shadow-neon-blue/30 scale-105 rotate-2"
    >
...
export default function GearChain({ lessonMode, studioBrainResponse, onGearUpdate }: GearChainProps) {
  ...
  useEffect(() => {
    if (studioBrainResponse) {
      const suggestions = GearService.extractGearFromResponse(studioBrainResponse)
  ...
  const getGearColor = (type: GearItem['type']) => {
    const colorMap = {
      amp: lessonMode ? 'bg-neon-cyan/20 border-neon-cyan/40 text-neon-cyan' : 'bg-neon-purple/20 border-neon-purple/40 text-neon-purple',
    ...
    <Card className="bg-glass-bg backdrop-blur-xl border border-glass-border rounded-xl shadow-2xl">
      <CardHeader className="pb-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mt-4">
          <Button
            variant={gearChains.selectedMode === 'studiobrainSuggestion' ? 'default' : 'outline'}
            size="sm"
          ...
          <Button
            variant={gearChains.selectedMode === 'customChain' ? 'default' : 'outline'}
            size="sm"
          ...
          <div className="space-y-4">
            {gearChains.studioBrainChain.length > 0 ? (
              ...
              <div className="text-center py-8">
                <Zap className={`w-12 h-12 mx-auto mb-4 ${lessonMode ? 'text-neon-cyan' : 'text-neon-purple'} opacity-50`} />
                <p className="text-slate-400">
                  Ask StudioBrain for tone recommendations to populate your gear chain
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            {gearChains.customChain.length > 0 ? (
              ...
              >
                ...
                >
                  {activeId ? (() => {
                    const activeItem = gearChains.customChain.find(item => item.id === activeId)
                    return activeItem ? (
                      <DragOverlayContent
                        item={activeItem}
                        lessonMode={lessonMode}
              ...
              <div className="text-center py-8">
                <Settings className={`w-12 h-12 mx-auto mb-4 ${lessonMode ? 'text-neon-cyan' : 'text-neon-purple'} opacity-50`} />
