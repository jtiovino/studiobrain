...
export function postValidatePlan(
  ...
  if (total !== input.time_minutes) throw new Error(`Minutes sum (${total}) ≠ time_minutes (${input.time_minutes})`);
  ...
  if (!first.includes(expected)) throw new Error(`First 'what_you_need' must be "Guitar in ${input.tuning}"`);

  // 3) Resource sanity: metronome only if BPM usage appears
  const listsMet = plan.what_you_need.some(s => /metronome/i.test(s));
  const mentionsBpm = plan.steps.some(st => /\b(bpm|metronome|quarter|eighth|triplet|sixteenth)\b/i.test(st.instructions));
  if (listsMet && !mentionsBpm) throw new Error("Metronome listed but no BPM usage in steps");

  // 4) Lesson mode fields must match toggle
  if (!input.lesson_mode) {
    const bad = plan.steps.find(st => st.why !== null || st.success_cue !== null);
    if (bad) throw new Error("lesson_mode=false but 'why'/'success_cue' not null");
  }
}

// Safe parse helper that combines JSON parsing, Zod validation, and post-validation
export function parsePlan(jsonStr: string, input: { time_minutes: number; tuning: string; lesson_mode: boolean }) {
