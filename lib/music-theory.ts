...
export type ModeName = 'ionian' | 'dorian' | 'phrygian' | 'lydian' | 'mixolydian' | 'aeolian' | 'locrian' | 'harmonicMinor' | 'melodicMinor'
export type ChordQuality = 'major' | 'minor' | 'diminished' | 'augmented' | 'dominant7' | 'major7' | 'minor7' | 'diminished7' | 'half-diminished7' | 'sus2' | 'sus4' | 'add9' | 'minor9' | 'power'
export type RomanNumeral = 'I' | 'II' | 'III' | 'IV' | 'V' | 'VI' | 'VII' | 'i' | 'ii' | 'iii' | 'iv' | 'v' | 'vi' | 'vii' | 'i°' | 'ii°' | 'iii°' | 'iv°' | 'v°' | 'vi°' | 'vii°'
...
export interface ChordInfo {
  ...
  intervals: number[]
  name: string
  function: 'tonic' | 'subdominant' | 'dominant' | 'mediant' | 'submediant' | 'leading-tone' | 'supertonic'
}
...
export const CHROMATIC_NOTES: NoteName[] = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']
...
export const MODES: Record<ModeName, ModeInfo> = {
  ...
  harmonicMinor: {
    ...
    characteristic: 'Raised 7th - exotic, Middle Eastern flavor, strong leading tone',
    parentScale: 'minor',
  ...
  melodicMinor: {
    ...
    characteristic: 'Raised 6th and 7th - smooth melodic motion, jazz applications',
    parentScale: 'minor',
...
export const CHORD_FUNCTIONS: Record<number, 'tonic' | 'subdominant' | 'dominant' | 'mediant' | 'submediant' | 'leading-tone' | 'supertonic'> = {
  0: 'tonic',
  1: 'supertonic',
  2: 'mediant',
  3: 'subdominant',
...
// Utility functions
export function getNoteFromSemitone(rootNote: NoteName, semitones: number): NoteName {
...
export function getScaleNotes(root: NoteName, mode: ModeName): NoteName[] {
  ...
  return modeInfo.intervals.map(interval => getNoteFromSemitone(root, interval))
}
...
export function buildTriadFromScaleDegree(scaleNotes: NoteName[], degree: number): NoteName[] {
  ...
  console.log(`    🔨 Building triad for degree ${degree}: indices [${rootIndex}, ${thirdIndex}, ${fifthIndex}] → [${root}, ${third}, ${fifth}]`)
...
export function getRomanNumeral(degree: number, quality: ChordQuality): RomanNumeral {
  const romanNumerals = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII']
  const lowerRomanNumerals = ['i', 'ii', 'iii', 'iv', 'v', 'vi', 'vii']
  const diminishedRomanNumerals = ['i°', 'ii°', 'iii°', 'iv°', 'v°', 'vi°', 'vii°']
...
export function getModeChords(root: NoteName, mode: ModeName): ChordInfo[] {
  ...
  return scaleNotes.map((_, index) => {
    ...
    console.log(`🎵 Degree ${scaleDegree}: [${triad.join(', ')}] → ${quality} → ${chordRoot}${quality === 'minor' ? 'm' : quality === 'diminished' ? '°' : ''}`)
...
export function getCharacteristicIntervals(mode: ModeName): { interval: number, name: string, quality: string }[] {
  const characteristics: Record<ModeName, { interval: number, name: string, quality: string }[]> = {
    ionian: [
    ...
    lydian: [
      { interval: 5, name: '4th', quality: 'augmented' }
    ],
    mixolydian: [
...
export function getCommonProgressions(mode: ModeName): { name: string, numerals: string[], description: string }[] {
  const progressions: Record<ModeName, { name: string, numerals: string[], description: string }[]> = {
    ionian: [
      { name: 'I-V-vi-IV', numerals: ['I', 'V', 'vi', 'IV'], description: 'Pop progression' },
      { name: 'ii-V-I', numerals: ['ii', 'V', 'I'], description: 'Jazz turnaround' },
      { name: 'I-vi-ii-V', numerals: ['I', 'vi', 'ii', 'V'], description: 'Circle progression' }
    ],
    dorian: [
      { name: 'i-IV-i', numerals: ['i', 'IV', 'i'], description: 'Modal vamp' },
      { name: 'i-VII-i', numerals: ['i', 'VII', 'i'], description: 'Dorian cadence' },
      { name: 'i-ii-i', numerals: ['i', 'ii', 'i'], description: 'Minor ii emphasis' }
    ],
    phrygian: [
      { name: 'i-II-i', numerals: ['i', 'II', 'i'], description: 'Phrygian cadence' },
      { name: 'i-VI-VII-i', numerals: ['i', 'VI', 'VII', 'i'], description: 'Spanish progression' }
    ],
    lydian: [
      { name: 'I-II-I', numerals: ['I', 'II', 'I'], description: 'Lydian characteristic' },
      { name: 'I-iii-II-I', numerals: ['I', 'iii', 'II', 'I'], description: 'Floating progression' }
    ],
    mixolydian: [
      { name: 'I-VII-I', numerals: ['I', 'VII', 'I'], description: 'Rock progression' },
      { name: 'I-VII-IV-I', numerals: ['I', 'VII', 'IV', 'I'], description: 'Mixolydian loop' }
    ],
    aeolian: [
      { name: 'i-VI-VII-i', numerals: ['i', 'VI', 'VII', 'i'], description: 'Minor progression' },
      { name: 'i-iv-V-i', numerals: ['i', 'iv', 'V', 'i'], description: 'Harmonic minor feel' }
    ],
    locrian: [
      { name: 'i°-II-i°', numerals: ['i°', 'II', 'i°'], description: 'Unstable resolution' }
    ],
    harmonicMinor: [
      { name: 'i-V-i', numerals: ['i', 'V', 'i'], description: 'Strong resolution with major V' },
      { name: 'i-VI-vii°-i', numerals: ['i', 'VI', 'vii°', 'i'], description: 'Harmonic minor cadence' }
    ],
    melodicMinor: [
      { name: 'i-IV-V-i', numerals: ['i', 'IV', 'V', 'i'], description: 'Major IV and V chords' },
      { name: 'i-ii-V-i', numerals: ['i', 'ii', 'V', 'i'], description: 'Jazz minor progression' }
    ]
  }
