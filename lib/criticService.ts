...
export async function reviewResponse({
  ...
  try {
    ...
    const userGearContext = userSettings?.gear ? [
      ...
      userSettings.gear.plugins?.length ? `Plugins: ${userSettings.gear.plugins.join(', ')}` : null,
      userSettings.gear.guitar?.length ? `Guitars: ${userSettings.gear.guitar.join(', ')}` : null,
      userSettings.gear.pedals?.length ? `Pedals: ${userSettings.gear.pedals.join(', ')}` : null,
      userSettings.gear.interface ? `Interface: ${userSettings.gear.interface}` : null,
      userSettings.gear.monitors ? `Monitors: ${userSettings.gear.monitors}` : null,
    ].filter(Boolean).join('\n') : 'No specific gear information provided'

    const contextInfo = lessonMode ? 'Educational mode is ON - explanations should include "why"' : 'Quick mode - keep responses concise'
...
Evaluate the response and return the final version (either unchanged or critic-adjusted):`
    ...
    const criticResponse = completion.choices[0]?.message?.content || assistantResponse
    const wasCriticAdjusted = criticResponse.includes('(Critic-adjusted)')

    return {
      finalResponse: criticResponse,
      wasCriticAdjusted,
      criticReason: wasCriticAdjusted ? 'Response adjusted for accuracy and safety' : undefined
    }
...
export async function shouldSkipCritic(originalPrompt: string, assistantResponse: string): Promise<boolean> {
  ...
  const mentionsGear = /\b(plugin|pedal|amp|daw|interface|compressor|eq|reverb|delay)\b/i.test(assistantResponse)
